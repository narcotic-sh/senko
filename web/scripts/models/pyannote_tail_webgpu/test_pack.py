...
class LayoutTests(unittest.TestCase):
    def test_matrix_round_trip_with_classifier_padding(self) -> None:
        matrix = np.arange(5 * 7, dtype=np.float16).reshape(5, 7)
        packed, shape = pack_matrix_i_o4(matrix)
        self.assertEqual(shape, [5, 2, 4])
        np.testing.assert_array_equal(unpack_matrix_i_o4(packed, matrix.shape), matrix)
...
class RealModelTests(unittest.TestCase):
    def test_b8_package_is_complete_reproducible_and_small(self) -> None:
        first_binary, first = build_package(MODEL, "tail.bin")
        second_binary, second = build_package(MODEL, "tail.bin")
        ...
        self.assertLess(first["binary"]["byte_length"], 104 * 1024)
